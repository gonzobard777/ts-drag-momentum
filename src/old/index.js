function AnimationLoop() {
    var animations = [], animating = true, frame;
    function animate() {
        if (frame) {
            return;
        }
        ...
        while (i--) {
            if (!animations[i] || animations[i]() === false) {
                animations.splice(i, 1);
            }
        }
        frame = null;
    }
    function add(animateFn) {
        animations.push(animateFn);
    }
...
var drag = {
    ...
    getPosition: function (e) {
        ...
        var event = (e ? e.touches ? e.touches[0] : e : {}), pos = { x: event.pageX, y: event.pageY, time: Date.now() };
